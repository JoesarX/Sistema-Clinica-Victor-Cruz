...
function HistorialCita() {
    ...
    const generatePDFInca = (incapacidadData, paciente, edad, fecha) => {
        console.log(incapacidadData)
        ...
        doc.setFont("times");
        doc.setFontSize(16);
        doc.text("Dr. Victor Manuel Cruz Andino", 8.5 / 2, 0.5, { align: "center" });

        // Text "ESPECIALISTA SALUD OCUPACIONAL" in bold and a different font
        doc.setFont("helvetica", "bold");
        doc.setFontSize(10);
        doc.text("ESPECIALISTA SALUD OCUPACIONAL", 8.5 / 2, 0.75, { align: "center" });

        // Text "Medico Cirujano" in bold
        doc.text("Médico Cirujano", 8.5 / 2, 0.9, { align: "center" });

        // Text "Epidemiologo * Salubrista" in bold
        doc.text("Epidemiologo * Salubrista", 8.5 / 2, 1.05, { align: "center" });

        // Text "Colonia Kennedy al final de la Calle Comercial, contiguo a BANRURAL"
        doc.setFont("times", "normal");

        doc.text("Colonia Kennedy al final de la Calle Comercial, contiguo a BANRURAL", 8.5 / 2, 1.20, { align: "center" });

        // Text "Telefono: 2230-3901, Celular: 9689-4453"
        doc.text("Teléfono: 2230-3901, Celular: 9689-4453", 8.5 / 2, 1.35, { align: "center" });

        doc.setFont("times");

        doc.text("Nombre:", 1.50, 1.80);
        doc.text([paciente], 3.2, 1.78);
        ...
        doc.line(2.1, 1.80, 6, 1.80); // Line for Name
        doc.line(6.6, 1.80, 7, 1.80); // Line for Age
        doc.line(2.1, 2.10, 6, 2.10); // Line for Date


        doc.text("Medicina General", 0.15, 1.5);
        doc.text("Control de Embarazo", 0.15, 2.5);
        doc.text("Control Niño Sano", 0.15, 3.5);
        doc.text("Nebulización", 0.15, 4.5);
        doc.text("Cirugía Menor", 0.15, 5.5);
        doc.text("Oxigenoterapia", 0.15, 6.5);




        doc.text("Horario:", 8, 0.5, { align: "right" });
        doc.text("Lunes a Viernes", 8, 0.65, { align: "right" });
        doc.text("de 8:00 a.m. a 6:00 p.m.", 8, 0.80, { align: "right" });
        doc.text("EMERGENCIAS.", 8, 1.1, { align: "right" });
        ...
        //x1,y1  x2,y2
        doc.setFontSize(10);
        doc.text("Incapacidad Médica", 1.5, 2.50);
        doc.autoTable({
            head: [['Tipo de Ausencia', 'Fecha Inicial', 'Cantidad de Dias']],
            body: [
                [type,
                    initial,
                    days]
            ...
            startY: 2.6, // Adjust the vertical position as needed
        ...
        addTextAndIncrementY("Comentarios", doc, 6);
        doc.setFont("helvetica", "normal");
        addTextAndIncrementYB(Comentarios, doc, 6);
        doc.setFont("helvetica", "bold");
        addTextAndIncrementY("Diagnóstico", doc, 6);
        doc.setFont("helvetica", "normal");
        addTextAndIncrementYB(Diagnostico, doc, 6);
        doc.setFont("helvetica", "bold");
        addTextAndIncrementY("Estudios de Gabinete", doc, 6);
        doc.setFont("helvetica", "normal");
        addTextAndIncrementYB(Estudios, doc, 6);
        doc.setFont("helvetica", "bold");
        addTextAndIncrementY("Procedimientos", doc, 6);
        doc.setFont("helvetica", "normal");
        addTextAndIncrementYB(Procedimientos, doc, 6);
        doc.setFont("helvetica", "bold");
        addTextAndIncrementY("Instrucciones Médicas", doc, 6);
        doc.setFont("helvetica", "normal");
        addTextAndIncrementYB(Instrucciones, doc, 6);
        doc.setFont("helvetica", "bold");
        addTextAndIncrementY("Historial de Medicamentos Actuales", doc, 6);
        doc.setFont("helvetica", "normal");
        addTextAndIncrementYB(MedicamentosActuales, doc, 6);
        ...
        doc.setLineWidth(0.01);
        doc.line(2, 10.5, 6.5, 10.5);

        // Text "Firma y Sello del Medico" at the bottom middle
        doc.setFontSize(10);
        doc.text("Firma y Sello del Médico", 8.5 / 2, 10.75, { align: "center" });
        ...
        doc.save('AusenciaMedica_' + paciente + '_' + fecha + '.pdf');
    };
    function addTextAndIncrementYB(text, doc, maxWidth) {
        var textLines = doc.splitTextToSize(text, maxWidth);
        doc.text(textLines, 1.5, startY);
        startY += 0.2 + (textLines.length * 0.2); 
    }
    function addTextAndIncrementY(text, doc, maxWidth) {
        var textLines = doc.splitTextToSize(text, maxWidth);
        doc.text(textLines, 1.5, startY);
        startY += 0.1 + (textLines.length * 0.1); 
      }
    var startY = 3.7
    ...
    const handleExportPDFInca = () => {
        ...
        generatePDFInca(incapacidad, paciente.nombre, paciente.edad, formatDate(paciente.fecha));
    };
    ...
    const maxDescriptionCharacters = 800;
    ////////////////////////////////////////////////////////////////////////////
                    ...
                    {showIncapacity && (
                        <div class='contenedor'>
                            ...
                            <div className="btn-group my-2" role="group">
                                ...
                                <label className="btn btn-outline-dark" htmlFor="academica">Académica</label>
                            ...
                            <div class="row mb-3">
                                <div class="form-group col-md-12">
                                    ...
                                    <textarea maxLength={maxDescriptionCharacters} class="form-control" id="comentarios" rows="5" placeholder="Escriba aquí" onChange={(e) => setComentarios(e.target.value)}></textarea>
                                </div>
                                <div>
